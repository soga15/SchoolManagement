    ...
    <template v-slot:top>
      ...
      >
        ...
        <v-dialog v-model="dialogView" max-width="500px">
          <v-card>
            <v-card-title class="text-h5">VIEW USER</v-card-title>
            <v-card-text>
               <v-row>
                  <v-col
                    cols="12"
                  ...
                  >
                    <h3>Role_ID: {{editedItem.role_id}}</h3>
                  </v-col>
                  ...
                  >
                    <h3>RoleName: {{editedItem.role && editedItem.role.name}}</h3>
                  </v-col>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Role_Name', value: 'role.name' },
    ...
    methods: {
      ...
      updateItem(item) {
        axios.put(`api/user/update/${item.id}`, item)
        .then(response=> {
      ...
      deleteItemConfirm() {
        axios.delete(`api/user/delete/${this.editedItem.id}`)
        .then(response =>{
          console.log(response)
        }).catch(error =>{
          console.log(error)
